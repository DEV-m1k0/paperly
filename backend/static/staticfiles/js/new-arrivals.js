document.addEventListener("DOMContentLoaded", () => {
  const productsGrid = document.getElementById("newArrivalsProducts");
  ...
  function buildCard(product, index) {
    ...
    const chipText = discountPercent > 0 ? `-${discountPercent}%` : "Новинка";
  ...
  async function loadNewArrivals() {
    ...
    productsGrid.innerHTML = `<p class="empty-state">Загрузка новинок...</p>`;

    try {
      const response = await fetch("/api/products/?newest_days=3&ordering=-created_at");
      if (!response.ok) {
        productsGrid.innerHTML = `<p class="empty-state">Не удалось загрузить новинки.</p>`;
        return;
      }

      const rows = parseRows(await response.json()).filter((item) => item && item.id);
      if (!rows.length) {
        productsGrid.innerHTML = `<p class="empty-state">За последние 3 дня новинок нет.</p>`;
        return;
      }
      ...
      productsGrid.innerHTML = `<p class="empty-state">Ошибка загрузки новинок.</p>`;
    }
  }
