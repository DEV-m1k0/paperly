document.addEventListener("DOMContentLoaded", () => {
  const productsGrid = document.getElementById("bestsellersProducts");
  ...
  function buildCard(product, index) {
    ...
    const chipText = discountPercent > 0 ? `-${discountPercent}%` : "Хит";
  ...
  async function loadBestsellers() {
    ...
    productsGrid.innerHTML = `<p class="empty-state">Загрузка хитов продаж...</p>`;

    try {
      const response = await fetch("/api/products/?bestseller_days=3&ordering=-sold_recent");
      if (!response.ok) {
        productsGrid.innerHTML = `<p class="empty-state">Не удалось загрузить хиты продаж.</p>`;
        return;
      }

      const rows = parseRows(await response.json()).filter((item) => item && item.id);
      if (!rows.length) {
        productsGrid.innerHTML = `<p class="empty-state">За последние 3 дня продаж не было.</p>`;
        return;
      }
      ...
      productsGrid.innerHTML = `<p class="empty-state">Ошибка загрузки хитов.</p>`;
    }
  }
